...
// Cloud mask function using the SCL (Scene Classification Layer) band
function maskS2clouds(image) {
  var scl = image.select('SCL');
  // Keep only: 4 (vegetation), 5 (bare soil), 6 (water), 11 (snow) -
  // i.e. mask out 3 (cloud shadow), 8/9 (clouds), 10 (cirrus)
...
print('Pre-flood S2 candidate dates and cloud %:',
  s2PreCollection.aggregate_array('system:index'));
print('Pre-flood S2 candidate cloud percentages:',
  s2PreCollection.aggregate_array('CLOUDY_PIXEL_PERCENTAGE'));

// Build a cloud-free composite by taking the median across all dates
var s2Pre = s2PreCollection.median().clip(aoi);

print('Pre-flood Sentinel-2 composite info:', s2Pre);

Map.addLayer(s2Pre, {bands: ['B4', 'B3', 'B2'], min: 0, max: 3000}, 'Pre-flood S2 (True Color)');

// ---- 4. SENTINEL-2 (optical) - POST-FLOOD ----
var s2PostCollection = ee.ImageCollection('COPERNICUS/S2_SR_HARMONIZED')
  .filterBounds(aoi)
  .filterDate(postFloodStart, postFloodEnd)
  .filter(ee.Filter.lt('CLOUDY_PIXEL_PERCENTAGE', 80))
  .map(maskS2clouds);

print('Number of post-flood S2 candidates found:', s2PostCollection.size());

var s2Post = s2PostCollection.median().clip(aoi);

print('Post-flood Sentinel-2 composite info:', s2Post);
